...
class NearbyLocationsViewModel: ViewModelBase {
  private let db = Firestore.firestore()
  private let manager = CLLocationManager()
  
  var location: Observable<[Location]> {
    return manager.rx.location.compactMap({ $0 }).flatMap({ self.findNearby(latitude: $0.coordinate.latitude, longitude: $0.coordinate.longitude, range: 0.01) })
  }
  ...
  private func findChannel(latitude: Double, longitude: Double, range: Double) -> Observable<[Location]> {
    let location = CLLocation(latitude: latitude, longitude: longitude)
